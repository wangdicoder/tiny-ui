...
const Popover = ({
  ...
  const { onVisibleChange, overlay, className, style, children } = restProps;
  ...
  useEffect(() => {
    ...
    if (triggerRef.current) {
      ...
      triggerRef.current.style.display =
        'style' in child ? (child as HTMLElement).style.display || 'inline-block' : 'inline-block';
    }
  ...
  return (
    <>
      <div className={`${prefixCls}__trigger`} ref={triggerRef}>
        {children}
      </div>
      <Portal>
        <CSSTransition
